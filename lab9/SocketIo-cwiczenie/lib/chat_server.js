...
var assignGuestName = function (socket,guestNumber,nickNames,namesUsed){
    ...
    nickNames[currentRoom[socket.id]].push({'nick': nick, 
        'user': socket.id, 'socket': socket});
        ...
        socket.on('msg', function(data) {
            ...
            for(var i =0; i < nickNames[currentRoom[socket.id]].length; i++){
                if(nickNames[currentRoom[socket.id]][i] &&
                    currentRoom[nickNames[currentRoom[socket.id]][i].user] 
                    ...
                    nickNames[currentRoom[socket.id]][i].socket.emit('newMsg', 
                        newData);
    ...
    socket.on('changeNick', function(data) {
        ...
        if(nickExists){
            ...
            if(userNo.i){
                ...
                nickList.push({'nick': newNick, 'user': socket.id, 'socket': socket});
            }
        ...
        if(nickNames[data]){
            ...
            for(var i = 0; i < nickList.length; i++){
                ...
                if(nickList[i] && nickList[i].nick === 
                    currentNick[socket.id] && nickList[i].user !== socket.id)
                {
                    nickExists = true;
                }
            }
            if(userExists){
                ...
                if(!nickExists){
                    // socket.join(data);
                    nickList.push({'nick': currentNick[socket.id], 
                        'user': socket.id, 'socket': socket});
                    currentRoom[socket.id] = data;
                }else {
                    // socket.join(data);
            ...
            nickNames[data] = [];
            nickNames[data].push({'nick': currentNick[socket.id], 
                'user': socket.id, 'socket': socket});
            namesUsed.push(data);
            currentRoom[socket.id] = data;
         ...
         socket.emit('joinResult', {room: currentRoom[socket.id], 
            nick: currentNick[socket.id] , roomsList: namesUsed });
      ...
      for(var i =0; i < namesUsed.length; i++){
        for(var j=0; j < nickNames[namesUsed[i]].length; j++){
            if(nickNames[namesUsed[i]][j] && 
                nickNames[namesUsed[i]][j].user === socket.id){
